...
- ✅ **Отчество оператора** добавлено. Все реквизиты (ФИО, ИНН, ОГРНИП, адрес, e-mail)
  централизованы в `src/i18n/utils.ts` → `company` и подставляются на всех страницах.
  ⚠️ Отчество проставлено как «Владимирович» — **подтвердите/поправьте** одно поле
  `owner.patronymicRu/En` в `utils.ts`.
